...
#### 배열을 이용한 큐의 특징: (배열을 이용한 큐의 특징과 동일한 내용)
...
- 또한, 배열을 이용한 큐와 마찬가지로, 덱큐를 선형적인 덱큐로 구현했을 경우 요소들이 뒤로 쏠리기 때문에 이러한 문제를 효율적으로 극복하고자 원형 형태로 구현하는데 이 구현이 고려해야 할 점도 많고 조금 복잡하다.
